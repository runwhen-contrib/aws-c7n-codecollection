...
List security group that allow public IP ingress in AWS Region `${AWS_REGION}` in AWS account `${AWS_ACCOUNT_ID}` 
    ...
    ...    cmd=jq -r '["Security-Group-Id", "Security-Group-Name", "Open-Ports", "VPC-ID"], (.[] | [ .GroupId, .GroupName, (.IpPermissions[0].FromPort | tostring) + "-" + (.IpPermissions[0].ToPort | tostring) + "/" + .IpPermissions[0].IpProtocol, .VpcId ]) | @tsv' ${OUTPUT_DIR}/aws-c7n-network-health/sg-insecure-ingress/resources.json | column -t

    RW.Core.Add Pre To Report    ${parsed_results.stdout} 
        ...
        FOR    ${dir}    IN    @{dir_list}
            ...
            IF    len(@{resource_list}) > 0
                FOR    ${item}    IN    @{resource_list}
                    RW.Core.Add Issue        
                    ...    severity=3
                    ...    expected=AWS Security-Group `${item['GroupId']}` in AWS Region `${AWS_REGION}` in AWS account `${AWS_ACCOUNT_ID}` should not have a rule to allow public IP access
                    ...    actual=AWS Security-Group `${item['GroupId']}` in AWS Region `${AWS_REGION}` in AWS account `${AWS_ACCOUNT_ID}` allow public IP access rule
                    ...    title=Detected public IP access rule in AWS security group `${item['GroupId']}` in AWS region `${AWS_REGION}` and AWS account `${AWS_ACCOUNT_ID}`
                    ...    reproduce_hint=${c7n_output.cmd}
                    ...    details=${item}
                    ...    next_steps=Remove public IP access from AWS Security Group in AWS region \`${AWS_REGION}\` and AWS account \`${AWS_ACCOUNT_ID}\`
